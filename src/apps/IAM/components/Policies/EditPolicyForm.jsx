...
const EditPolicyForm = ({ selected, onCloseModal }) => {
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (groupRole?.status) {
        notify("Política eliminada satisfactoriamente");
      } else {
        notifyError(`Error al eliminar política: ${groupRole?.msg}`);
      }
    } catch (err) {
      notifyError(err);
    }
  ...
  return (
    <div className="flex flex-col justify-center items-center mx-auto">
      <h1 className="text-2xl my-4">Eliminar política</h1>
      ...
      <Form onSubmit={onSubmit} grid>
        <ButtonBar>
          <Button type={"submit"}>Eliminar política</Button>
        </ButtonBar>
      </Form>
    </div>
  );
