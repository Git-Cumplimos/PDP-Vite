          ...
          if (!res?.status) {
            ...
            if (res?.obj?.respuesta == "Consulta con error") {
              if (res?.obj?.data?.nombre == "Usuario no existe") {
                notifyError(
                  "Error respuesta Practisistemas:(Consulta invalida[Usuario no existe])"
                );
              } else if (
                res?.obj?.data?.reply ==
                ...
                notifyError(
                  "Error respuesta Practisistemas:(Consulta invalida [Error consultando Matrícula, posiblemente no existe])"
                );
              } else if (
                res?.obj?.data?.reply == "Error en el codigo de municipio"
              ) {
                notifyError(
                  "Error respuesta Practisistemas:(Consulta invalida[Error consultando Círculo, posiblemente no existe])"
                );
              }
            } else {
  ...
  return (
    <Fragment>
      ...
      <h1 className="text-3xl mt-6">
        Compra Pin:{" "}
        {state?.desc == "Certificado TL"
          ? "Certificado de Tradición y Libertad (SNR)"
          : state?.desc == "Historico Vehicular"
          ? "Histórico Vehicular"
          : state?.desc}{" "}
      </h1>
