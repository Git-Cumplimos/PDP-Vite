...
import { ErrorCustomBackend, fetchCustom } from "../utils/fetchAlmaseg";
const sleep = (millisecons) => {
  return new Promise((resolve) => setTimeout(resolve, millisecons));
};
export const useFetchAlmasegTrx = (
  url_trx_ = "",
  url_consulta_ = "",
  name_ = ""
) => {
  const [loadingState, setState] = useState(false);
  const fetchAlmasegTrx = useCallback(
  ...
  return { loadingState, fetchAlmasegTrx };
};
