...
const DevolucionPinesOlimpia = () => {
  ...
  const onSubmitConsultPin = (e) => {
    ...
    fetchDataPinesCea(
      ...
      .then((res) => {
        ...
        if (!res?.status || (res?.obj?.result?.Estado != 'Recaudado o Activo para Enrolar')) {
          if (res?.obj?.result?.Estado == undefined){
            ...
            setCodigoAprobacion("");
            setFechaTransaccion("");
            notifyError("Error Respuesta Olimpia: Estado de pin no permitido para para anulación");
            setNumeroPin("");
            setNumeroDocumento("");
            setTipoDocumentoInput(1);
            return;
        ...
        if (!res?.status) {
          ...
          fetchDataPinesCea(
            ...
            .catch((err) => {
              ...
              setTipoDocumentoInput(1);
              // console.error(err);
              notifyError("Error Respuesta Olimpia: Error confirmando anulación del pin cea Olimpia");
            });
        }
      })
      .catch((err) => {
        ...
        setTipoDocumentoInput(1);
        // console.error(err);
        notifyError("Error Respuesta Olimpia: Error anulando el pin cea Olimpia");
      });
  };
  ...
  return (
    <Fragment>
      ...
      <h1 className='text-3xl mt-6 mb-10'>Anulación Pines CEA Olimpia</h1>
