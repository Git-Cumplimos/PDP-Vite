...
export const descriptionErrorFrontGeneral: string =
  "Error respuesta Front-end PDP: (%s)";
    ...
    if (peticion_?.status === false && peticion_?.obj?.error_status === true) {
      ...
      if (Object.keys(peticion_?.obj?.error_msg).length > 0) {
        ...
        error_msg_console = errorNameKey
          .map((nameKey) => {
            return `* ${nameKey}= ${peticion_?.obj?.error_msg[nameKey].comment} (${peticion_?.obj?.error_msg[nameKey].blocker})`;
          })
          .join("\n");
      }
