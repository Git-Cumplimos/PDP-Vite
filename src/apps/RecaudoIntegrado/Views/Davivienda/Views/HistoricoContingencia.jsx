...
const HistoricoContingencia = () => {
  ...
  useEffect(() => {
    fetchData(
      "http://127.0.0.1:5000/servicio-contingencia-empresarial-pdp/search?fecha_inicio_inicio=2023-01-17&fecha_inicio_fin=2023-12-19&identificador_banco=davivienda",
      "GET",
      {},
      {},
      {},
      ...
      .then((res) => {
        setCantidadPaginas(res?.obj?.maxPages);
        //    console.log(res?.obj?.results["results"])
        setDatosTablaContingencia(res?.obj?.results["results"]);
      })
      .catch((err) => {
        console.log(err);
  ...
  useEffect(() => {
    if (fechaInicial && fechaFinal) {
      fetchData(
        ...
        `http://127.0.0.1:5000/servicio-contingencia-empresarial-pdp/search?fecha_inicio_inicio=${fechaInicial}&fecha_inicio_fin=${fechaFinal}&identificador_banco=davivienda`,
        "GET"
      )
