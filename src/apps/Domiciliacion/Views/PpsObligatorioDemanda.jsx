...
const PpsObligatorioDemanda = ({ ced }) => {
  ...
  const [numPlanilla, setnumPlanilla] = useState(null);
  ...
  useEffect(() => {
    infoTicket(datosRespuesta?.[0]?.["inserted_id"], 108, tickets)
      .then((resTicket) => {
      ...
      if (tipoComercio === "OFICINAS PROPIAS") {
        ...
        if (valorAportar >= 5000 && valorAportar <= 149000) {
          fetchData(
            ...
            `http://127.0.0.1:7000/CrearPlanillaDemandaObligatorioOfPropias`,
            "POST",
            {},
            {
              Identificacion: numDocumento,
              PlanillaCode: numPlanilla,
              trazabilityOperatorCode: numPlanilla,
        ...
        //       setEsPropio(true);
        //       if (String(numPlanilla).charAt(0) === "3") {
        //         if (valorAportar >= 5000 && valorAportar <= 149000) {
        //           fetchData(
